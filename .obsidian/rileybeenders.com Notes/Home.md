...
### Job Search Tracking
- [[Past Disney Applications]] — the 29 inactive Disney applications from the Workday portal (26 from before this site), with the four-month 10119309 interview timeline
...
- The tracker on `/more-info` has separate switches for the chart and the table (`ganttSection.chartVisible` / `tableVisible`); right now only the **table** is on. The Gantt chart + table exist in **two** hand-maintained places that can drift, `README.md` and `data/more-info/gantt.md`; they currently match. See [[More Info and Gantt Data]] and the Sync Charts procedure in [[Repository Agent Skills (.agents)]].
