...
`/about-this-site` is the site as its own case study — the one page that documents itself. It has its own nav tab, its own data file, its own Studio entry, and an agent procedure (`site-timeline-sync`) that keeps its numbers and timeline current — since 2026-09-19 automatically, through a GitHub Actions workflow that runs the procedure after every push to `main` (see [[Build Tooling and Config]]). Added 2026-09-17, built with the three `motion-*` skills as their first real use; revised 2026-09-18 to replace the pinned-screenshot section with live "Behind the site" demos and to show every capture in the theme the visitor is *not* using.
...
Each dot is a `<button aria-pressed>`; clicking (or `← → Home End` within the group) selects it, and the card below swaps with `AnimatePresence mode="wait"` (opacity + 10px, 0.36s). The card shows the formatted date, an era badge (Shipped / In progress / Planned), the short hash linked to `REPO_URL/commit/<hash>` (`lib/site.ts`), title, summary, tags, `files / +ins / −del`, and a thumbnail if the entry names a `screenshotId`.

**Stars and connectors** (added 2026-09-18 for the K2 Space pair). An entry may set `mark: "star"` — a fixed 18px dot for a moment whose weight isn't in its diff — and `linkFrom: "<id>"` naming an earlier entry's `id`. The layout then treats the pair as one unit: both dots go above the axis, a `.tl-link` line with an arrowhead runs from the earlier dot to the later one (drawn under the dots, `--from`/`--to` in axis %), captioned with `linkLabel` or, by default, the gap in days ("7 days"); the pair skips the depth-stacking that other dots use, so no stem crosses the line, and both ends hold their hover label higher. The card reads the relation in words ("**7 days** after Tailored resume submitted to K2 Space") and the mobile list marks the star with `tl-list-item--star`, so the connection survives without the axis. A connector only draws forward in time; a `linkFrom` that points at a later or unknown entry is ignored. The one pair today: "Tailored resume submitted to K2 Space" (2026-08-01, `id: k2-resume-out`) → ★ "K2 Space reached out to schedule interview" (2026-08-08).

**The tour** (added 2026-09-18). Left alone, the timeline plays through itself: it opens on the first entry and, once ≥35% of `.tl` is on screen at an axis-showing width (`matchMedia("(min-width: 861px)")`), rests on each entry for `DWELL_MS` (6s) then steps one dot right, wrapping at the end. The control sits at the right end of the note row (`.tl-head`): a `Playing · 04 / 25` label in tracked uppercase (mono count) and a 36px round button (`.tl-tour-btn`, `aria-label` Pause/Play the timeline, `data-state`) whose hairline circle is the track and whose ink arc fills clockwise from twelve as the dwell runs out — an SVG circle with `pathLength="1"`, so `strokeDashoffset = 1 − progress`, written per frame from a `requestAnimationFrame` loop (no React state per frame; frame deltas are capped at 100ms so a background tab doesn't skip entries). Pause holds the arc where it is and dims it to `--muted`; Play resumes from there. Choosing a dot (click, focus, or arrow keys) ends the tour on that entry and empties the ring; keyboard focus landing anywhere inside the timeline except the control pauses it, so the card can't swap out from under someone tabbing through it. The card slot is `aria-live="off"` while playing and `"polite"` once the visitor is driving. Under reduced motion nothing plays: a mount effect lands on the `present` entry (else the latest `past` one, as before) and leaves the control offering Play, so the tour is opt-in there. The control is `display: none` with the axis under 860px.

Under 860px the axis and card are hidden and `.tl-list` — a vertical list of every entry with a dot on a track — takes over; that list is also the reading order for assistive tech. The switching is CSS-only; the one JS media query is the tour's, which only decides whether the loop runs.
...
| `scripts/capture-site-screenshots.mjs` | Recaptures the page's screenshots with `playwright-core` driving the machine's Chrome/Edge (no download) at 1440×900 @2x, each in **both** themes via the real toggles (the nav switch; `#theme-toggle` in the Studio): home hero, full-page home, projects, this page's own timeline, and the Studio with this page open — ten files, `<name>-light.png` / `<name>-dark.png`. Hides the Next dev badge, waits for fonts and non-lazy images (bounded at 4s), removes the earlier single-theme files. Needs `npm run site` (both servers; the Studio shots are skipped if :3001 is down). Hides the Next dev badge and the Edit in Studio control. |
| `scripts/site-stats.mjs` | Computes the six stats from git and the repo, lists commits since the last hashed timeline entry with key-commit candidates and JSON skeletons; `--write` refreshes the stats in the data file by label; `--json` for machines. Counts skills under `.agents/skills/` (fixed 2026-09-19 after the move). |
| `scripts/check-about-site.mjs` | Validates the data file — dates in order, known eras, at most one `present`, every `hash` a real commit, connectors and `screenshotId`s that resolve, the six stats present — and exits 1 with a list otherwise. Run before committing, and by the workflow before it commits. |

## Data — `data/site/about-site.json`

Shape (`types/about-site.ts` → `AboutSiteData`): `hero { title, tagline }`, `dates` (`ProjectDates` — `2026-06-28`, `ongoing`), `summary`, `bullets` (`ResumeBullet[]`), `images` (`ProjectImage[]`), optional `caseStudy` (`ProjectAdditionalInfo`), `feature` (`ProjectFeature`: `intro`, `stats[]`, `timeline[]`, `pillars[]`, `screenshots[]`, `backend { eyebrow, intro, items[] }` — `backend.eyebrow` is the block's heading text, "Behind the site", not a label above one). The feature types live in `types/resume.ts` next to `Project` (`ProjectStat`, `TimelineEntry` with `era: "past" | "present" | "future"` plus the optional `id`, `mark: "dot" | "star"`, `linkFrom`, `linkLabel` for connectors, `FeaturePillar`, `FeatureScreenshot` with `src` + `srcDark`, `FeatureBackend` / `BackendItem` with `demo: "resume" | "studio" | "skills"`, `BackendMatch`, `BackendSkill`). Screenshots are served from `public/project-images/rileybeenders-com/` as `<name>-light.png` / `<name>-dark.png`. As of 2026-09-19 the timeline holds 26 entries (21 past, 1 present — "A design critic joins the repository" — and 4 planned), and the six stats read Commits 123 · Days in motion 83 · Agent skills 23 · Vault notes 35 · Applications tracked 16 · Projects queued 8.

Edited in the Studio under **About this site** (rail group after More Info; `aboutSite` in `server.mjs` `FILES` and `schema.js`), which round-trips the file byte-for-byte. Refreshed by the `site-timeline-sync` procedure — locally on request, and unattended after every push to `main` by `.github/workflows/site-timeline-sync.yml`, whose bot commits ("Update the site timeline after …" / "Refresh the site stats after …") are the only writes to this file that don't come from the Studio or a session — see [[Repository Agent Skills (.agents)]].

## Nav

`BpNav` gained `{ label: "About this site", href: "/about-this-site", gradient: true }` — the tab is drawn in a slow gradient-text sweep. The impeccable critique flagged it on 2026-09-18 and the decision was to flatten it, but that edit never reached `main`, so the sweep is still live; the phone nav is two rows with a scrolling link row. See [[Blueprint Nav and Mark]]. More Info's "Read more" link now points here (`/about-this-site`, rendered as a `next/link` since it's a site path).
