...
The repo doubles as the owner's personal job-search system of record. This note covers the non-code folders and files that track it; the on-site rendering of this data is covered in [[More Info and Gantt Data]] and [[BulletList GanttChart and JobsTable]].

## The three folders

| Folder | Contents |
|---|---|
| `JobsAppliedTo/` | The **original job posting** PDF for each application, numbered `001`–`013`, named `<ID>_<Job Title> at <Company> - <date/slug>.pdf`. |
| `output/ApplicationsUsed/` | The **actual tailored resume/cover letter** submitted for each application — the deliverable of the `custom-resume` agent skill. Includes matching cover letters (`.docx` + `.pdf`) for the most recent applications (011, 012, 013). |
| `output/pdf/` | A handful of additional generated resume PDF variants (Disney Principal Software Engineer, SpaceX Sr. Network Security Engineer, and two versions of the Disney Ride Control resume) — earlier or alternate outputs from the same tailoring workflow. |
| `references/` | Dated **snapshot resumes** (`RileyBeenders_Apr2022.pdf`, `_Sept2025.pdf`, `_Jan2026.pdf`) kept as historical evidence of how the resume has evolved, and as reference material the `custom-resume` skill can draw layout/content cues from. |
...
Each entry in `output/ApplicationsUsed/` was produced by the `custom-resume` agent skill (`.agents/SKILL.md`), not by the site's own `/api/resume-pdf` route. Workflow summary (full detail in [[Repository Skills (.agents SKILL.md)]]):

1. Collect page count + which `JobsAppliedTo/*.pdf` to target.
