...
| `/more-info` | `app/(site)/more-info/page.tsx` | About copy + "Read the full story" link to `/about-this-site` + [[GanttChart JobsTable and gantt.ts\|GanttChart + JobsTable]] (the tracker section only while `ganttSection.visible` — currently off) | "More Info \| Riley Beenders" |
...
Because it uses `fs`, this page can't be statically exported without the file present at build time (fine on Vercel). **Today `ganttSection.visible` is `false`** (set 2026-09-16), so the live page stops after section 02 and never touches `gantt.md`. See [[More Info and Gantt Data]].
