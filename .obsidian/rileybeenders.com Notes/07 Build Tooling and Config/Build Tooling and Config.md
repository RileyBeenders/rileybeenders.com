...
3. `node scripts/site-stats.mjs` for the log, then `anthropics/claude-code-action@v1` with `anthropic_api_key: ${{ secrets.ANTHROPIC_API_KEY }}` (the commented alternative is `claude_code_oauth_token` for a Claude subscription), a fixed `prompt` that names the push range and points at the skill's **Unattended runs** section, and `claude_args` of `--max-turns 40` plus an `--allowedTools` list limited to reading, editing, the two scripts, and read-only git (`log`, `show`, `diff`, `status`, `rev-list`). Claude cannot commit, push, or run anything else.
