...
The repo's only workflow. On every push to `main` (and via *Run workflow*) it runs the `site-timeline-sync` procedure unattended through Claude Code and commits the result:

1. `actions/checkout@v4` with `fetch-depth: 0` (the stats count every commit and the validator checks hashes), `actions/setup-node@v4` on Node 22 — no `npm ci`, the scripts have no dependencies.
...
3. `node scripts/site-stats.mjs` for the log, then `anthropics/claude-code-action@v1` with `claude_code_oauth_token: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}` (a token from `claude setup-token`, so runs bill to the Claude subscription; the commented alternative is `anthropic_api_key` for API billing), a fixed `prompt` that names the push range and points at the skill's **Unattended runs** section, and `claude_args` of `--max-turns 40` plus an `--allowedTools` list limited to reading, editing, the two scripts, and read-only git (`log`, `show`, `diff`, `status`, `rev-list`). Claude cannot commit, push, or run anything else.
4. `node scripts/check-about-site.mjs` — a failing validator fails the job before anything is committed.
5. Commit and push, if anything changed: `git add` of `data/site/about-site.json` and the vault folder only, author `github-actions[bot]`, subject "Update the site timeline after \<sha\>" when a `hash` / `title` / `era` line changed, otherwise "Refresh the site stats after \<sha\>", then `git push origin HEAD:main` and a job summary. Pushes made with `GITHUB_TOKEN` never start another workflow run, so the sync can't loop; `github.actor != 'github-actions[bot]'` is a second guard, and a `concurrency` group queues runs so a second push waits for the first run's commit.
