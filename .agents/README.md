...
| [`site-timeline-sync`](skills/site-timeline-sync/SKILL.md) | After a meaningful commit or merge; "update the site timeline / about page", "refresh the site stats"; when `/about-this-site` looks stale — and **automatically after every push to `main`** (`.github/workflows/site-timeline-sync.yml` runs it through Claude Code and commits the result) | Keeps `data/site/about-site.json` current: runs `scripts/site-stats.mjs` to refresh the six stats from git and list commits since the timeline's last entry, adds only KEY commits as brief timeline entries, moves the present marker, promotes/prunes planned items, validates the file (`scripts/check-about-site.mjs`), recaptures screenshots (`scripts/capture-site-screenshots.mjs`) when the UI changed (local runs only), then hands off to `vault-sync`. |
