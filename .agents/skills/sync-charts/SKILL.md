...
This is a narrower, chart-only counterpart to `vault-sync` (`.agents/skills/vault-sync/SKILL.md`). If the winning content also needs to reach the Obsidian vault notes, the normal `vault-sync` rules still apply after the mismatch is fixed.
