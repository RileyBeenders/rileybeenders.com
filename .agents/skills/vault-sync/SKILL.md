...
Any change under: `app/`, `components/`, `data/`, `lib/`, `types/`, `ResumeBuilder/`, `scripts/`, `studio/`, `assets/`, `public/`, `design/`, `README.md`, `README_TODO.md`, `package.json`, `next.config.mjs`, `tsconfig.json`, `PRODUCT.md`, `DESIGN.md`, `.impeccable/`, or `.agents/` itself. Changes confined to `2.JobsApplliedTo/`, `1.ApplicationsUsed/`, `output/`, `references/`, or `tmp/` only need a vault update if they add/remove a tracked job application (update `06 Job Search Tracking/Job Application Tracker.md` and see **Job application detail pages** below) — routine PDF file drops don't.
...
| `app/(site)/projects/page.tsx` (as the route's behaviour, not just its metadata), `app/(site)/projects/projects.css`, `components/projects/ProjectEntry.tsx`, `ProjectListSpine.tsx`, `ProjectGallery.tsx`, `Lightbox.tsx`, `CaseStudy.tsx`, `ProjectDateBox.tsx`, `lib/projects.ts` | `02 Components/Projects Route (BpComingSoon).md` and `01 Routes and Pages/Routes Overview.md` |
| `app/(site)/about-this-site/**`, `components/about-site/**`, `components/projects/ProjectFeature.tsx`, `components/projects/feature/**`, `components/blueprint/CountUp.tsx`, `WordReveal.tsx`, `ScrollWords.tsx`, `lib/dates.ts`, `lib/useInViewOnce.ts`, `lib/useSpotlight.ts`, `lib/site.ts`, `types/about-site.ts`, `data/site/**`, `scripts/**`, `public/project-images/rileybeenders-com/**` | `02 Components/About This Site Page.md` (plus `01 Routes and Pages/Routes Overview.md` for the route row and `03 Data Layer/Data Layer and Types.md` for type changes) |
| `studio/**` (the Studio itself: `server.mjs` `FILES`, `ui/schema.js` schemas or `RAIL`) | The note for whichever data file the change edits (e.g. `About This Site Page.md` for `aboutSite`, `Career Content.md` for projects/proofs), and `07 Build Tooling and Config/Build Tooling and Config.md` if how the Studio runs changes |
...
| `app/base.css`, `app/(site)/blueprint.css`, `design/**`, `lib/palette.ts`, `lib/palettes.ts`, `lib/fonts.ts` | `05 Styling and Design/Design System (Blueprint Press).md` — and if the change touches a token (a color, ramp weight, font role, radius, spacing step, easing), the repo-root `DESIGN.md` + `.impeccable/design.json` must be regenerated in the same session (`$impeccable document`, or edit the frontmatter by hand for a one-value change) so the detector keeps comparing against the truth |
| `DESIGN.md` (repo root), `.impeccable/design.json` | `05 Styling and Design/Design System (Blueprint Press).md` (it states that the root file is the machine-readable spec and summarizes its named rules) and `08 Agents and Automation/Impeccable Design Workflow.md` |
| `PRODUCT.md`, `.impeccable/critique/**`, `.impeccable/config.json`, `.agents/skills/impeccable/**` | `08 Agents and Automation/Impeccable Design Workflow.md` (product truth summary, the standing interview outcomes, the critique history), plus the `.agents` note's skill list if the skill folder itself changes |
...
| `package.json`, `next.config.mjs`, `tsconfig.json`, `.gitignore`, `scripts/**` (as tooling) | `07 Build Tooling and Config/Build Tooling and Config.md` |
...
`sync-charts` (`.agents/skills/sync-charts/SKILL.md`) is a narrower, chart-only counterpart: it keeps the Job Application Tracker gantt block + table identical between `README.md` and `data/more-info/gantt.md`. If a chart edit also needs to reach the vault notes, run that first, then apply the rules above.
